...
<div class="d-block">
    ...
    <div class="d-block">
        <small class="media-heading text-muted btn-block margin-zero">
            {{trans('misc.created')}} {{ date($settings->date_format, strtotime($response->date)) }}
            <span class="align-middle mx-1" style="font-size:8px;">|</span>
...
@if (isset( $deadline ) && $deadline > $timeNow && $response->finalized == 0)
 	<div class="btn-group btn-block margin-bottom-20 @if( Auth::check() && Auth::user()->id == $response->user()->id ) d-none @endif">
        ...
        <a href="{{ route('donation.payment_options', ['id' => $response->id]) }}" class="btn btn-main btn-primary no-hover btn-lg btn-block custom-rounded">
            {{ trans('misc.donate_now') }}
        </a>
