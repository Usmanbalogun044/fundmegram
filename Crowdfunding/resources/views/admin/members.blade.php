            ...
            @endif
               ...
               @if ($data->total() !=  0 && $data->count() != 0)
                  <tr>
                                        ...
                                        <th class="active">verified_Action</th>
                 ...
                 @foreach ($data as $user)
									 <tr>
                                        ...
                                        @endif
                                            <form action="{{ route('admin.toggleVerification', $user->id) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                        
                                                <input type="hidden" name="is_verified" value="{{ $user->is_verified ? 0 : 1 }}">
                                                
                                                <button type="submit" class="btn {{ $user->is_verified ? 'btn-success' : 'btn-secondary' }} btn-sm">
					...
					<form method="POST" action="{{ route('user.destroy', $user->id) }}" id="form{{ $user->id }}" class="d-inline">
                        ...
                        <a href="{{ route('admin.userdetails', $user->id) }}" class="btn btn-success rounded-pill btn-sm me-2">
                            <i class="bi-pencil">view details</i>
                        </a>
					</form>
				 </div>
