...
class AdminController extends Controller
{
	...
	public function withdrawalsPaid(Request $request)
	{
		...
		$sender     = $this->settings->email_no_realy;
		...
		Mail::send(
			...
			function ($message) use ($sender, $fullNameUser, $titleSite, $_emailUser) {
				$message->from('noreply@appointme.me', $titleSite)
	...
	public function postEditCampaigns(Request $request)
	{
        ...
        if ($statusGlobal === 'active') {
            ...
            Mail::send('emails.campaignapproved', [
                'username' => $sql->user()->name,
                'title_site' => $_title_site,
                'amount' => $sql->goal,
                'email' => $sql->user()->email,
                'campaign' => $sql->title,
            ], function ($message) use ($_title_site, $sql) {
                $message->from('noreply@appointme.me', $_title_site);
                $message->to($sql->user()->email, $sql->user()->name);
                $message->subject('Your Campaign Has Been Approved');
            });
        }
	...
	public function deleteCampaign(Request $request)
	{
		...
		$data->delete();
        $_title_site = $this->settings->title;
        Mail::send('emails.campaigndeleted', [
            'username' => $data->user()->name,
            'title_site' => $_title_site,
            'amount' => $data->goal,
            'email' => $data->user()->email,
            'campaign' => $data->title,
        ], function ($message) use ($_title_site, $data) {
            $message->from('noreply@appointme.me', $_title_site);
            $message->to($data->user()->email, $data->user()->name);
            $message->subject('Your Campaign Has Not Been Approved');
        });
	...
	public function approveDonation(Request $request)
	{
        ...
        $campaign = Campaigns::findOrFail($sql->campaigns_id);
		$sql->approved = '1';
		$sql->save();
        $_title_site = $settings->title;
        Mail::send('emails.donationapproved', [
            'username' => $sql->fullname,
            'title_site' => $_title_site,
            'amount'=>$sql->donation,
            'email'=>$sql->email,
            ...
            $message->from('noreply@appointme.me', $_title_site);
            $message->to($sql->email,$sql->fullname);
            $message->subject('donations approved');
        });
	...
	public function deleteDonation(Request $request)
	{
        ...
        $_title_site = $settings->title;
		$sql->delete();
        Mail::send('emails.donationdelete', [
            'username' => $sql->fullname,
            'title_site' => $_title_site,
            'amount'=>$sql->donation,
            'email'=>$sql->email,
            ...
            $message->to($sql->email,$sql->fullname);
            $message->subject('donations not approved');
        });
    ...
    public function updateStripeSettings(Request $request, $id)
    {
        $data = PaymentGateways::findOrFail($id);
        $request->validate([
            'fee' => 'required|numeric|min:0',
            'fee_cents' => 'required|numeric|min:0',
            'key' => 'required|string',
    ...
    public function updatewiretransferSettings(Request $request, $id)
    {
        $data = PaymentGateways::findOrFail($id);
        $request->validate([
            'fee' => 'required|numeric|min:0',
...
// dd($data);
        ...
        return back()->with('success_message', 'paypal settings updated successfully!');
    }
    ...
    public function showDetails($id)
{
    ...
    // If you want to load the user's related data, you can use eager loading
    $user->load('campaigns'); // Assuming you want to show campaigns or other relationships
...
{
    $user = User::findOrFail($id);
    $user->is_verified = !$user->is_verified; // Toggle status
    $user->save();

    return back()->with('success_message', 'User verification status updated!');
}
public function emailtoalluser(Request $request){
        $request->validate([
        ...
        foreach($users as $user){
            ...
            return back()->with('success', 'Emails sent to all users successfully!');
    ...
    public function sendEmailToUser(Request $request)
    {
        ...
        Mail::to($request->email)->send(new AdminNotificationMail($request->subject, $request->message));
