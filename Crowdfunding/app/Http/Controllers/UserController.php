...
class UserController extends Controller
{
	...
	public function uploadPassportAndNationalID(Request $request)
{
    $settings = AdminSettings::first();
    $id = Auth::user()->id;
    ...
    // Validate inputs
    $validator = Validator::make($request->all(), [
        'passport_front' => 'required|mimes:jpg,gif,png,jpe,jpeg|dimensions:min_width=125,min_height=125|max:' . $settings->file_size_allowed,        'national_id_front' => 'required|mimes:jpg,gif,png,jpe,jpeg|dimensions:min_width=125,min_height=125|max:' . $settings->file_size_allowed,
        'national_id_back' => 'required|mimes:jpg,gif,png,jpe,jpeg|dimensions:min_width=125,min_height=125|max:' . $settings->file_size_allowed,
    ]);
  ...
  if ($validator->fails()) {
    ...
    if ($request->hasFile('passport_front')) {
        $passportFront = $request->file('passport_front');
        $passportFrontName = time() . '_passport_front.' . $passportFront->getClientOriginalExtension();
        $passportFront->move(public_path('passport'), $passportFrontName);
        $user->passport_front = 'passport/' . $passportFrontName;
    }


    // National ID Front Upload
    if ($request->hasFile('national_id_front')) {
        $nationalIdFront = $request->file('national_id_front');
        $nationalIdFrontName = time() . '_national_id_front.' . $nationalIdFront->getClientOriginalExtension();
        $nationalIdFront->move(public_path('national_id'), $nationalIdFrontName);
        $user->national_id_front = 'national_id/' . $nationalIdFrontName;
    }

    // National ID Back Upload
    if ($request->hasFile('national_id_back')) {
        $nationalIdBack = $request->file('national_id_back');
        $nationalIdBackName = time() . '_national_id_back.' . $nationalIdBack->getClientOriginalExtension();
        $nationalIdBack->move(public_path('national_id'), $nationalIdBackName);
        $user->national_id_back = 'national_id/' . $nationalIdBackName;
    }

    // Save user record with updated file paths
    $user->save();

   return redirect('/register/address')->with('success', 'Passport and National ID (Front & Back) uploaded successfully.');
